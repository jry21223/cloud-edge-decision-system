...
| P4 多边缘节点 | 注册、心跳、Peer 调度、冲突仲裁 | **进行中** | 已实现双 Edge、心跳、Peer 调度与仲裁；待 Docker 实测和冲突实验 |
...
当前已完成：`POST /v1/arbitrate` 的 DREAM-Fuse 证据仲裁，将校准置信度、节点可靠度、观测新鲜度、时空一致性和版本信息组合；Recorder 分开计算原始冲突率、自主形成结果率与带真值正确解决率。双 Edge Compose 配置包含心跳注册，Controller 使用 DREAM-Route 按 deadline、准确率、可用率、时延和负载对 Peer/Cloud/Fallback 排序。尚未完成：Docker 运行实测、普通任务路径自动触发多 Peer 融合、冲突数据集与 5%/90% 指标实验。
